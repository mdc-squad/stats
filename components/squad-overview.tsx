    ...
    <div className="space-y-4">
      ...
      <Card className="border-christmas-gold/20 bg-card/60">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="h-[360px] w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 12, right: 16, left: 0, bottom: 8 }} onClick={handleChartClick}><CartesianGrid stroke="rgba(148, 163, 184, 0.18)" strokeDasharray="3 3" /><XAxis dataKey="date" axisLine={{ stroke: "#94a3b8" }} tickLine={{ stroke: "#94a3b8" }} tick={{ fill: "#cbd5e1", fontSize: 11 }} tickMargin={8} /><YAxis axisLine={{ stroke: "#94a3b8" }} tickLine={{ stroke: "#94a3b8" }} tick={{ fill: "#cbd5e1", fontSize: 11 }} width={42} /><RechartsTooltip content={<SquadChartTooltip />} wrapperStyle={{ pointerEvents: "auto" }} />{visibleChartLines.map((line) => <Line key={line.key} type="monotone" dataKey={line.key} name={line.label} stroke={line.color} strokeWidth={2.5} dot={false} activeDot={{ r: 5, stroke: line.color, fill: line.color }} connectNulls />)}</LineChart></ResponsiveContainer></div>
