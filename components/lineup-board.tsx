...
> = {
  ...
  WHITE: { border: "border-slate-100/52", text: "text-slate-50", glow: "from-slate-100/36 via-slate-200/14 to-transparent", accent: "bg-slate-100", panel: "bg-slate-100/16", rowBorder: "border-slate-100/34", rowHover: "hover:border-slate-50/62" },
}
...
const LINEUP_PLAYER_METRICS: LineupPlayerMetric[] = [
  ...
  { key: "avgRevives", label: "Поднятия", tooltip: "Поднятия, в среднем за игру", icon: "revives", getValue: (player) => player.totals.avgRevives, digits: 1 },
  { key: "avgHeals", label: "Хил", tooltip: "Хил, в среднем за игру", icon: "heals", getValue: (player) => player.totals.avgHeals, digits: 1 },
  { key: "avgDowns", label: "Ноки", tooltip: "Ноки, в среднем за игру", icon: "downs", getValue: (player) => player.totals.events > 0 ? player.totals.downs / player.totals.events : 0, digits: 1 },
  { key: "avgKills", label: "Убийства", tooltip: "Убийства, в среднем за игру", icon: "kills", getValue: (player) => player.totals.events > 0 ? player.totals.kills / player.totals.events : 0, digits: 1 },
  { key: "avgDeaths", label: "Смерти", tooltip: "Смерти, в среднем за игру", icon: "deaths", getValue: (player) => player.totals.events > 0 ? player.totals.deaths / player.totals.events : 0, digits: 1 },
...
function sanitizeLineupFileName(value: string) {
  return String(value || "lineup")
    .trim()
    .replace(/[\\/:*?"<>|]+/g, "-")
    .replace(/\s+/g, "-")
    .replace(/-+/g, "-")
...
function getLineupSideLabel(side: LineupSideKey) {
  return side === "siteOne" ? "side-1" : "side-2"
}
...
export function LineupBoard({ games = [], players = [], onOpenPlayer }: LineupBoardProps) {
  ...
  const exportRefs = useRef<Record<LineupSideKey, HTMLDivElement | null>>({ siteOne: null, siteTwo: null })
  ...
  const canExportLineup = hasAnyLineupContent && !isInitialLoading
  ...
  }, [downloadDataUrl, exporting, lineup])
        ...
        {lineup ? (
          <div className="pointer-events-none fixed left-[-10000px] top-0" aria-hidden="true">
            {LINEUP_SIDE_KEYS.map((sideKey) => {
              ...
              const exportTitleMeta = splitMatchTitle(exportTitle)
              const exportOpponent = calendarGame?.opponent?.trim() ?? ""
              const exportVisibleSquads = SQUAD_ORDER.filter((squadName) => hasSquadContent(exportSide[squadName] ?? []))

              return (
                <div
                  key={`export-${sideKey}`}
                  ref={(node) => {
                    exportRefs.current[sideKey] = node
                  }}
                  ...
                  <div className="mb-5 rounded-2xl border border-christmas-gold/25 bg-[#070a12] px-6 py-5 text-center">
                    ...
                    {exportTitleMeta.details.length > 0 || exportOpponent ? (
                      <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                        {exportTitleMeta.details.map((detail) => (
                          <span
                            key={detail}
