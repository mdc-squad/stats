...
- Кэш хранится в `localStorage` (`mdc-api-cache-v4`).
...
- Синхронизация запускается:
  ...
  - кнопкой `Обновить API (Shift+R)`,
  - кнопкой `Сбросить кэш`.
- `Сбросить кэш` очищает локальный кэш и запускает полную повторную синхронизацию.
- При `Обновить API`/`Сбросить кэш` используется `publish=true` (мимо кэша upstream API).
